...
(function ($) {
  ...
  $.fn.pagination = function (options) {
    ...
    return this.each(function (index, element) {
      ...
      var $menu = $content.parent().find('.' + opts.menuClass); //show page 
      ...
      function showPage(whichPage) {
        ...
        getPageList(totalPages, currentPage, paginationSize).forEach(function (item) {
          $("<li>").addClass("current-page").toggleClass("active", item === currentPage).append($("<a>").attr({
      ...
      $menu.append($("<li>").addClass("previous-page").append($("<a>").attr({
        href: "javascript:void(0)"
      })), $("<li>").addClass("next-page").append($("<a>").attr({
        href: "javascript:void(0)"
      })));
      $content.show();
      showPage(1);
...
$(document).ready(function () {
  ...
  $(window).scroll(function () {
    var header = $('#header');
    var mediaQuery = matchMedia("(min-width: 61.999em)").matches;
  ...
  function carousel_play(id, carousel) {
    ...
    $prevButton.addClass('invisible'); // update selected cellButtons

    carousel.on('select.flickity', function () {
      $cellButtons.filter('.is-selected').removeClass('is-selected');
      $cellButtons.eq(flkty.selectedIndex).addClass('is-selected');
      $cellLabels.filter('.is-selected').removeClass('is-selected');
      $cellLabels.eq(flkty.selectedIndex).addClass('is-selected');
  ...
  carousel_play($id_homepage_carousel, $carousel_homepage); //carousel product page
  ...
  carousel_play($details__images_carousel, $carousel_product_details_page); //carousel product details page
  ...
  carousel_play($details__project_carousel, $carousel_project_details_page); //load more product in product_category
  ...
  load_more_responsive('.product__line-wrapper', 'load_more'); // zoom product detail
